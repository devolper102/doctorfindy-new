...
{
    protected $fillable = ['user_id','doctor_id','slots','online_slots','message','status','price','video_consultation_price'];
    use Cachable;
    /**
    ...
    public function hospitalapp()
    {
        return $this->belongsTo('App\User', 'user_id')->with('profile',function ($q){
                return $q->select('user_id','avatar');})->withDefault();
    }
    ...
    public function doctor()
    {
     ...
     public function team_doctor()
    {
        return $this->belongsTo('App\User', 'doctor_id')->select('id','first_name','last_name','slug','email','location_id')
            ...
            })->with('specialities')->with('location','doc_teams','feedbacks','onlines');
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function saveAdminAppointmentLocation($request, $user_id = '')
    {
        $doctor_id = !empty($user_id) ? $user_id : $request['id'];
        $this->doctor_id = $doctor_id;
        $this->message = 'message';
        $request_slots = array();
        $services = !empty($request['services']) ? $request['services'] : '';
