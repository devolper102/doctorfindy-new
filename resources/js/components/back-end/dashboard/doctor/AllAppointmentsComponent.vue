      ...
      <div id="content">
        ...
        <div class="container">
            ...
            <div class="tab-pane fade show active" id="today_appointment" role="today_appointment" aria-labelledby="today-appointment">
              <h2>Total Appointments:<span style="color:green"> {{doctor.appointments.length}}</span></h2>
              <table id="today_table" class="table table-bordered dt-responsive nowrap tabs_table" style="width:100%" v-if="doctor.appointments.length > 0">
                <thead>
                <tr>
                  <th>Patient</th>
                  <th>Hospital</th>
                    ...
                    <td>
                      <span v-if="appointment.hospital_id !== 0">
                       <a :href="'/profile/'+appointment.hospital_profile.slug" class="float-left mr-2">
                         <img  v-if="appointment.hospital_profile.profile.avatar" v-lazy="'/uploads/users/'+appointment.patient_id+'/'+appointment.hospital_profile.profile.avatar" class="img-fluid w-30px h_30 rounded-circle" :alt="appointment.hospital_profile.first_name +' '+ appointment.hospital_profile.last_name">
