<template>
  <div>
     <div class="container mb-3 p-0" id="wish-list">
      <p class="pb-2 p-2 pl-lg-3 font-weight-bold border-bottom-dark">Wish list</p>
    </div>
    <div class="row" v-if="savedDoctors.length >= 1">
       <div class="col-lg-6" v-if="savedDoctors.length >= 1">
        <div class="box_radius box_shadow p-3">
          <div class="saved_items">
            <p class="pb-2 border-bottom-dark font-weight-bold">Doctor you saved <span v-if="filteredDoctors.length >= '1'">({{ filteredDoctors.length }})</span>
              <span v-else>( 0 )</span>
            </p>
          </div>
       ...
       <div class="col-lg-6" v-if="savedDoctors.length >= 1">
        <div class="box_radius box_shadow p-3">
          <div class="saved_items">
            <p class="pb-2 border-bottom-dark font-weight-bold">Hospital you saved <span v-if="filteredHospitals.length >= '1'">({{ filteredHospitals.length }})</span>
...
<script>
export default {
  name: 'PatientSavedSectionComponent',
  ...
  created () {
    console.log('All saved items',this.patient.profile.saved_doctors)
    let self = this
    if (this.patient.profile.saved_doctors) {
      self.savedDoctors = JSON.parse(this.patient.profile.saved_doctors)
    }
      ...
      console.log(self.filteredDoctors)
    }
    if (this.savedDoctors !== null && this.savedDoctors.length > 0) {
