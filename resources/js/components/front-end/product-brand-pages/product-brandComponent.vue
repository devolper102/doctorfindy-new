        ...
        <b-row>
          <b-col cols="12">
            <div class="product-banner-heading w-100 d-inline-block text-center">
              <h2 class="text-white font-weight-bold text-size-32 text-xs-25">{{current_brand.name}}</h2>
            </div>
          </b-col>
          <b-col cols="12">
    ...
    <div class="scroll-cart-drop w-100 d-inline-block">
     ...
     <div v-else>
      ...
      <div class="heading w-100 d-inline-block">
        ...
        col md ="6">
           <div class="product-categorie-select w-100 d-inline-block">
             <multiselect @input="changeFilter()" v-model="selectedFilter" :options="filterOpt" label="name" :allow-empty="false" placeholder="">
                <template slot="caret">
                <div class="multiselect__select">
                  <b-icon icon="chevron-down" variant="secondary">
                  </b-icon>
...
components: 
  ...
  props:['products','min_price','max_price','categories','current_brand','cities','brands','top_products','top_categories'],
data(){
    return{
      options:[],
      best_salon_heading:'',
  ...
  methods:
     {
      ...
      changeCity(){
        this.best_salon_heading = "Best "+this.current_brand.name+" Products in "+this.selectedCity.title;
      },
      changeFilter(){
        axios.get('/filter_brand_products/'+this.selectedFilter.id+'/'+this.current_brand.id+'/'+this.selectedFilter.name).then(response=>{
          this.allProducts = response.data.data;
          this.rows = response.data.total;
        });
      },
