<template>
  <div>
   ...
   :style="{ 'background-image': 'url(' + basePath + '/uploads/settings/home/' + hospital_img + ')' }" v-if="hide_show === 'true'">
      <div class="bg-list-doctor-in-pakistan w-100 d-inline-block">
        <div class="container">
          ...
          <div class="row">
            <div class="col-12">
              <div class="bg-title-doctor-in-pakistan w-100 d-inline-block">
                <h1 class="text-white text-center font-weight-bold text_30">{{heading}}</h1>
              </div>
            </div>
          </div>
           <div class="row">
...
export default {
name: "BannerHospitalSectionComponent",
props:['managements','segments','bread_crumb_data', 'fileSystemDriver'],
 data() {
    ...
    created() {
      ...
      this.hospital_banner = this.managements.find(pf =>pf.meta_key ==='city_wise_hospital_section')
      this.heading = JSON.parse(this.hospital_banner.meta_value).heading;
      this.hospital_img = JSON.parse(this.hospital_banner.meta_value).hidden_city_wise_hospital_img;
      this.hide_show = JSON.parse(this.hospital_banner.meta_value).show_city_wise_hospital_sec;
