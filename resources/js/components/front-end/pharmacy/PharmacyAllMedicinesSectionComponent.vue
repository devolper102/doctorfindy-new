<template>
  <div id="all-medicines">
    <section class="pharmacy-all-medicines-section mt-5 mb-5">
      <div class="container">
        <div class="row">
          <div class="col-12">
            <h2 class="text-center font-weight-bold text_24 mb-4">Explore all Medicines</h2>
          </div>
        </div>
            ...
            <div class="medicine-card bg-white box-shadow rounded overflow-hidden h-100 position-relative">
              <a :href="medicine.slug ? '/pharmacy/medicine/' + medicine.slug : '#'" 
                ...
                <div class="medicine-details p-3">
                  ...
                  <p class="text_12 text-muted mb-1" v-if="medicine.category">
                    <i class="fa fa-tag mr-1"></i>{{ medicine.category.name }}
                  </p>
        ...
        <div class="row" v-else>
          <div class="col-12 text-center">
            <p class="text-muted">No medicines available at the moment.</p>
          </div>
        </div>
      </div>
    </section>
...
<script>
export default {
  name: "PharmacyAllMedicinesSectionComponent",
  ...
  methods: {
    ...
    loadPage(page) {
      if (this.medicines && page >= 1 && page <= this.medicines.last_page) {
        window.location.href = `/pharmacy?page=${page}`;
      }
    }
  }
}
