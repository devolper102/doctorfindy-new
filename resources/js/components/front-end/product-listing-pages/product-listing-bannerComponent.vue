        ...
        <b-row>
          <b-col cols="12">
            <div class="product-banner-heading w-100 d-inline-block text-center">
              <h2 class="text-white font-weight-bold text-size-32 text-xs-25">{{current_category.name}}</h2>
            </div>
          </b-col>
          <b-col cols="12">
      ...
      <div class="heading w-100 d-inline-block">
        ...
        col md ="6">
           <div class="product-categorie-select w-100 d-inline-block">
             <multiselect @input="changeCity()" v-model="selectedCity" :options="cityOpt" label="title" :allow-empty="false" placeholder="">
                <template slot="caret">
            ...
            <b-row v-if="allProducts.length>0" class="mt-4 mb-4 monthly-offer-product">
              ...
              <a :href="'/product-detail/'+product.slug">
                  ...
                  fluid :alt="product.name+' - photo'" class="w-100"></b-img>
                    ...
                    <p class="mt-2 mb-2 text_black text-size-15">
                      {{product.name}}
                    </p>
                    ...
                    <p class="color-date mt-2 mb-2 text-size-15 w-100 d-inline-block">{{product.short_description}}</p>
                ...
                <b-col cols="12">
                    <div class="empty-product-image w-100 d-inline-block mt-4 text-center">
                      ...
                      <p class="text-black font-weight-bold text-size-14">
                        No Products Available Against {{current_category.name}} Category</p>
                    </div>
                </b-col>
            </b-row>
            <b-row>
...
components: 
  ...
  name: "product-listing-bannerComponent",
  ...
  methods:
     {
      ...
      getMoreProducts(page){
        axios.get('/get_category_products_list/'+this.current_category.id+'?page='+page).then(response=>{
          this.allProducts = response.data.data;
          this.rows = response.data.total;
        });
      },
      changeCity(){
        this.best_salon_heading = "Best "+this.current_category.name+" Products in "+this.selectedCity.title;
      },
      changeFilter(){
        axios.get('/filter_category_products/'+this.selectedFilter.id+'/'+this.current_category.id+'/'+this.selectedFilter.name).then(response=>{
          this.allProducts = response.data.data;
          this.rows = response.data.total;
        });
      },
      ...
      makeActiveCurrentCategory(){
       ...
       getProductWithinPrice(){
        ...
        formData.append('category_id',this.current_category.id);
