        ...
        <div class="row mt-2 mb-4">
          <div class="col-112">
            <div class="heading-other-hispital">
              <h2 class="text-blue font-weight-bold text_20 text-xs-15">Top Hospitals in {{user.location.title}}</h2>
            </div>
          </div>
        </div>
        <div class="row">
          ...
          <div class="col-12">
              <div class="mt-4 more_btn text-center d-inline-block w-100">
                <a  href="javascript:void(0)" v-if="hospital_data.length > 8 && hospital_data.length > hospitalToShow-1" @click="showMoreHospitals()" class="d-inline-block rounded-pill text-center bg-blue btn_padding text-white text_14 text_md_12 mb-3"> View More hospitals
                </a>
            </div>
          </div>
        </div>
...
<script>
export default {
name: "TopHospitalsSectionComponent",
  props:['hospitals','user'],
  data() {
    return {
       hospitalToShow: 8,
       hospital_data:this.hospitals,
    }
  },
  created () {
  ...
  methods: {
    showMoreHospitals(){
         ...
         axios.get('/getMoreOtherHospital/'+this.user.id+'/'+'top').then(response=>{
