...
@section('title',ucfirst($user->first_name) .' '. ucfirst($user->last_name) .' Health Blogs') 

@section('meta_title',ucfirst($user->first_name) .' '. ucfirst($user->last_name) .' Health Blogs') 

@section('meta_description','All health blogs posted by '.ucfirst($user->first_name) .' '. ucfirst($user->last_name).'.')
    ...
    {{--End Start mobile-fixed-menu Section--}}
      ...
      <div class="row">
            <div class="col-12 mt-4">
                 {{ Breadcrumbs::render('article-profile-writer', $categories,$user) }}
            </div>
      </div>
    </div>

    {{--Start skin doctor detail--}}
    <profile
            :file-system-driver="{{ json_encode(env('FILESYSTEM_DRIVER')) }}"
            :user="{{json_encode($user, true)}}"
             :segments="{{ json_encode($segments, true) }}"
    ></profile>
    {{--End skin doctor detail --}}
