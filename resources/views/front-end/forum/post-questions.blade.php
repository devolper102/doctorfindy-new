...
@section('title','General Healthcare Discussion About '.$specialty->title. ' | DoctorFindy') 
@section('meta_title','General Healthcare Discussion About '.$specialty->title. ' | DoctorFindy') 
@section('meta_description','Healthcare discussion about '.$specialty->title)
@section('content')
<link rel="stylesheet" rel="preload" type="text/css" href="{{ asset('compiled/forum-speciality.min.css') }}" >
       @php
  ...
  <div class="container" v-cloak>
      <div class="row">
            <div class="col-12 mt-4">
              {{ Breadcrumbs::render('ask-a-doctor-online-speciality-slug', $specialty) }}
            </div>
      </div>
    </div>

    {{--Start section genral healthcare discussion--}}
    <forum-banner
    :segments="{{json_encode($segments, true) }}"
    ></forum-banner>
    {{--End section genral healthcare discussion--}}
    <forum
            :all-question="{{ $all_forums }}"
            :questions="{{ json_encode($forums, true) }}"
            :specialities = "{{json_encode($specialities, true)}}"
            :specialty = "{{json_encode($specialty, true)}}"
            :patient="{{ json_encode($logged_user, true) }}"
            :logged_user = "{{json_encode($active_user, true)}}"
    ></forum>
