...
<div id="search">
    ...
    @php
        ...
        $keyword = "";
         if($service != null && $location != null && $area != null)
        {
            $breadcrumbsdata=Breadcrumbs::generate('treatments-slug-city-area',$service,$location,$area);
        }
        else if($area != null && $location != null)
        {
            $breadcrumbsdata=Breadcrumbs::generate('doctors-by-city-area',$location,$area);
        }
        else
        {
            if(Request::path() == ('doctors/'.$location->slug))
            {
                $breadcrumbsdata=Breadcrumbs::generate('doctors-by-city', $location);
            }
            else
            {
                $breadcrumbsdata=[];
    ...
    <search-page
            ...
            :result-location="{{json_encode($location->slug, true) }}"
