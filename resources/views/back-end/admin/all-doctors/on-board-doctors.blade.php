                ...
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                    <div class="dc-dashboardbox">
                        <div class="dc-dashboardboxtitle dc-titlewithsearch dc-titlewithdel">
                            <h2>All Onboard Doctors</h2>
                        </div>
                        
                        @if ($users->count() > 0)
                            <div class="dc-dashboardboxcontent dc-categoriescontentholder">
                                <table id="on-board-doctors" class="table table-striped table-bordered dt-responsive nowrap table-responsive">
                                    <thead>
                                        <tr class=" border-top border-bottom">
                                            ...
                                            <th class="none  font-weight-bold text-14">Comission Percent</th>
                                    ...
                                    <tbody >
                                        ...
                                        <tr>
                                            ...
                                            @if($user_data->userMeta == '' || $user_data->userMeta == 'null' || $user_data->userMeta == null)
                                            <td class="text-14">-</td>
                                            @else
                                            <td class="text-14">{{$user_data->userMeta->commission}}</td>
                                            @endif
...
$(document).ready(function() {
      localStorage. setItem('clickCount', 1);
       var table = $('#on-board-doctors').DataTable({
        "bDestroy": true,
        "defaultContent": "-",
        "targets": "_all",
        "order": [[12, "desc"]]
      ...
      $(document).on('keyup','.search_field',function(){
            ...
            console.log('youtrsf',base+"/on-board-doctors-search")
            $.ajax({
               type: "GET",
               url: base+"/on-board-doctors-search",
                     ...
                     var edit_icon=
                      ...
                      if(item.status===0 || item.status==='' || item.status==='pending' || item.status=== null || item.status==='null')
                                  ...
                                  else
                                 {
                                  ...
                                  if(item.userMeta == '' || item.userMeta == 'null' || item.userMeta == null)
                                 {
                                    var commission='-';
                                 }
                                 else
                                 {
                                    var commission=item.userMeta.commission;
                                 }
