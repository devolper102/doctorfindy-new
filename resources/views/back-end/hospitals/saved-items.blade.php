                                    ...
                                    <div class="row">
                                        ...
                                        $saved_hospitals = [];
                                         foreach ($saved_doctors as $key => $user_id){
                                          ...
                                          if ($role_type === 'doctor') {
                                            $saved_alldoctor[] = $user_id;
                                              
                                          }
                                          else{
                                            $saved_hospitals[] = $user_id;
                                          }
                                          }
                                      }
                                       
                                        ?>
                                        @if (!empty($saved_alldoctor))
                                            @foreach ($saved_alldoctor as $key => $user_id)
                                                @php 
                                                            ...
                                                            <div class="dc-title">
                                                                @if (!empty($specialities))
                                                                    @foreach ($specialities as $key => $user_speciality)
                                                                        ...
                                                                        @if (!empty($speciality))
                    <a href="{{ url('/specialties/'.clean($speciality->slug)) }}" class="dc-docstatus">{{ html_entity_decode(clean($speciality->title)) }}</a>  
                                                                        @endif
                                                                    @endforeach
                                                                @endif
                                                                <h3>
                                            ...
                                            @endforeach
                                            @if ( method_exists($saved_alldoctor,'links') )
                                                {{ $saved_alldoctor->links() }}
                                            @endif
                                        @else
